window.FeedManagementPage = {
  ...
  createMosaicConfig(data = {}) {
    ...
    const legacyPosition = {
      ...
      targetPage: defaults.targetPage,
  ...
  renderEmbeddedComponentConfig(component, { tabReminderOptions = null, tabReminderNote = '', tabReminderReadonly = false } = {}) {
    ...
    if (component.type === 'normal-popup') {
      ...
      const routeConfig = window.ConfigurationSections.renderRouteConfig({
        ...
        route: { type: popup.routeType || 'protocol', targetPage: popup.targetPage, protocol: popup.routeProtocol, pid: popup.pid, selectedPid: popup.selectedPid, skipType: popup.skipType, description: popup.routeDescription },
        field,
        bindingAttribute: 'data-operation-popup-field',
    ...
    if (component.type === 'mosaic') {
      ...
      const routeConfig = window.ConfigurationSections.renderRouteConfig({
        prefix: 'feed-mosaic',
        route: {
          type: position.routeType || 'protocol', targetPage: position.targetPage, protocol: position.routeProtocol,
          pid: position.pid, selectedPid: position.selectedPid, skipType: position.skipType, description: position.routeDescription
        },
        field,
        bindingAttribute: 'data-feed-mosaic-field',
        fieldMap: { type: 'routeType', targetPage: 'targetPage', protocol: 'routeProtocol', pid: 'pid', selectedPid: 'selectedPid', skipType: 'skipType', description: 'routeDescription' }
      });
      const workspace = `<div class="home-showcase-workspace"><div class="feed-mosaic-canvas" aria-label="拼图配置"><div class="feed-mosaic-piece-list">${pieces}</div><span class="feed-mosaic-position-actions"><button class="feed-mosaic-position-add" type="button" data-feed-mosaic-position-add aria-label="添加位置">+</button><button class="feed-mosaic-position-remove" type="button" data-feed-mosaic-position-remove aria-label="删除选中位置"${mosaic.positions.length === 1 ? ' disabled' : ''}>×</button></span></div><div class="home-showcase-assets">${assetControl('上传图片', 'image', position.image)}${assetControl('暗黑模式', 'darkImage', position.darkImage)}</div>${routeConfig}${field('商城 ID', `<input class="control" data-feed-mosaic-field="mallId" value="${this.escape(position.mallId)}" placeholder="请输入商城 ID" />`)}${field('出站过程配置', `<div class="home-showcase-popup-row">${assetControl('出站弹窗 logo', 'popupLogo', position.popupLogo)}<input class="control" data-feed-mosaic-field="popupCopy" value="${this.escape(position.popupCopy)}" placeholder="请输入出站弹窗文案" /></div>`)}${field('登录要求', `<label class="home-showcase-login"><input type="checkbox" data-feed-mosaic-field="requiresLogin"${position.requiresLogin ? ' checked' : ''} />用户需登录</label>`)}</div>`;
    ...
    const renderAll = () => {
      ...
      root.querySelector('#feed-embedded-config-content').innerHTML = component
        ...
        : !singlePageMode && showTabConfigWhenNoComponent
          ...
          : '';
      window.ConfigurationSections.bindRouteConfig(root, 'operation-popup');
      window.ConfigurationSections.bindRouteConfig(root, 'feed-mosaic');
      applyEditState();
      window.BackofficeLayout.bindGlobalTooltips();
    };
  ...
  openOperationPopupConfigurationList({ title = '运营弹窗配置列表', state, onEdit } = {}) {
    ...
    const render = () => {
      ...
      if (!tableBody) return;
      tableBody.innerHTML = visibleRows.length
        ? visibleRows.map((row) => `<tr><td>${row.sequence}</td><td>${escape(row.name)}</td><td>${escape(row.position)}</td><td>${escape(row.sortValue)}</td><td>${escape(row.onlineStart)}</td><td>${escape(row.onlineEnd)}</td><td>${window.BackofficeLayout.statusTag(row.status)}</td><td>${escape(row.editor)}</td><td class="feed-resource-list-actions"><button class="text-button" type="button" data-operation-popup-list-edit="${escape(row.tabId)}:${escape(row.id)}">编辑</button></td></tr>`).join('')
