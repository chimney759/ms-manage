window.DetailTemplateStyleFormPage = {
  ...
  productCatalog(record) {
    ...
    return this.merchantsForTemplate(record).flatMap((merchant, merchantIndex) => productSeeds.filter((_, itemIndex) => itemIndex % Math.min(this.merchantsForTemplate(record).length, 3) === merchantIndex % Math.min(this.merchantsForTemplate(record).length, 3)).map(([id, title, brand, type, salesPrice, officialPrice, image]) => ({ id: `${merchant.id}-${id}`, productNo: id, title, supplierId: merchant.id, supplier: merchant.name, brand, type, image, salesPrice, officialPrice, cost: salesPrice, price: officialPrice, status: merchant.status === '下线' ? '已下线' : '上线中' })));
  },
  ...
  createComponent(type) {
    ...
    if (type === 'merchantProductFlow') return { id, type, title: '合作商货品', selectedProductIds: [] };
  ...
  render({ recordId = null } = {}) {
    ...
    return `<section class="content detail-template-style-page"><div class="page-heading"><div class="page-title-row"><button class="back-button" id="back-to-detail-templates" type="button" title="返回合作商详情页管理">‹</button><h1>模板样式编辑</h1></div><span class="heading-note">${this.escape(record.name)} · ${this.escape(scope)}</span></div>
      <section class="style-builder" id="style-builder" data-record-id="${this.escape(record.id)}">
        ...
        <aside class="style-config-panel"><div class="style-panel-heading"><h2>组件配置</h2><span id="style-config-type">未选择组件</span></div><div id="style-config-content" class="style-config-content"><div class="style-config-empty">从左侧拖入组件，或点击预览中的组件进行配置</div></div></aside>
      </section>
  ...
  bind({ navigate, recordId = null } = {}) {
    ...
    let components = Array.isArray(record.styleComponents) ? record.styleComponents.map((component) => {
    ...
    const renderConfig = () => {
      ...
      configType.textContent = component ? this.componentLabel(component.type) : '未选择组件';
    ...
    document.getElementById('save-detail-template-style-page').addEventListener('click', () => {
      ...
      window.localStorage.setItem(this.storageKey, JSON.stringify(records.map((item) => item.id === recordId ? { ...item, styleComponents: components, updatedAt } : item)));
