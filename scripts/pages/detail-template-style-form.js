window.DetailTemplateStyleFormPage = {
  ...
  createComponent(type) {
    ...
    if (type === 'search') return { id, type, placeholder: '搜索商品、品牌或优惠', hasBeenSaved: false };
    if (type === 'resource') return { id, type, name: '未命名资源位', layout: '拼图', materials: [this.createResourceMaterial()], targeting: this.createResourceTargeting(), hasBeenSaved: false };
    if (type === 'merchantProductFlow') return { id, type, title: '合作商货品', selectedProductIds: [], productCategories: {}, hasBeenSaved: false };
    return { id, type, title: '商品信息流', library: '', libraryId: '', libraryName: '', hasBeenSaved: false };
  },
  render({ recordId = null } = {}) {
    ...
    return `<section class="content detail-template-style-page"><div class="page-heading"><div class="page-title-row"><button class="back-button" id="back-to-detail-templates" type="button" title="返回合作商详情页管理">‹</button><h1>模板样式编辑</h1></div><span class="heading-note">${this.escape(record.name)} · ${this.escape(scope)}</span></div>
      <section class="style-builder" id="style-builder" data-record-id="${this.escape(record.id)}">
        ...
        <section class="style-preview-panel"><div class="phone-stage" id="phone-stage"><div class="phone-frame"><div class="phone-status"><span>9:41</span><span>● ● ●</span></div><div class="page-canvas" id="style-page-canvas" aria-label="页面预览区域"></div></div><div class="component-annotations" id="style-component-annotations" aria-label="页面结构标注"></div></div><div class="canvas-drop-tip" id="canvas-drop-tip">将功能组件拖入此处</div></section>
        <aside class="style-config-panel"><div class="style-panel-heading style-config-heading"><div><h2>组件配置</h2><span id="style-config-type">未选择组件</span></div><button class="button secondary style-remove-component-action" id="remove-style-component" type="button" hidden>移除组件</button></div>${record.type === '充值详情页' ? '<button class="text-button recharge-fixed-config-trigger" id="edit-recharge-fixed-style" type="button">固定样式配置</button>' : ''}<div id="style-config-content" class="style-config-content"><div class="style-config-empty">从左侧拖入组件，或点击预览中的组件进行配置</div></div></aside>
      </section>
  ...
  bind({ navigate, recordId = null } = {}) {
    ...
    const removeComponentAction = document.getElementById('remove-style-component');
    const supportsSearch = record.type === '电商详情页';
    const supportsMerchantProductFlow = record.type !== '电商详情页';
    let components = (Array.isArray(record.styleComponents) ? record.styleComponents : this.defaultComponents()).filter((component) => (supportsSearch || component.type !== 'search') && (supportsMerchantProductFlow || component.type !== 'merchantProductFlow')).map((component) => {
      if (component.type === 'merchantProductFlow') return { ...component, selectedProductIds: Array.isArray(component.selectedProductIds) ? component.selectedProductIds : [], productCategories: component.productCategories && typeof component.productCategories === 'object' ? component.productCategories : {}, hasBeenSaved: component.hasBeenSaved ?? true };
      if (component.type !== 'resource') return { ...component, hasBeenSaved: component.hasBeenSaved ?? true };
      return { ...component, materials: Array.isArray(component.materials) && component.materials.length ? component.materials : [this.createResourceMaterial()], targeting: this.normalizeResourceTargeting(component.targeting), hasBeenSaved: component.hasBeenSaved ?? true };
    });
    let draggedType = '';
    ...
    const removeNewComponent = (id) => {
      const component = components.find((item) => item.id === id);
      if (!component || component.hasBeenSaved) return false;
      removeComponent(id);
      return true;
    };
    ...
    const removeDraggedComponent = () => {
      ...
      if (component) {
        ...
        if (removeNewComponent(component.id)) window.BackofficeLayout.showToast('已移除组件', `${label}已从页面预览中移除`);
      }
      draggedComponentId = '';
    };
    ...
    const renderConfig = () => {
      ...
      const canRemoveNewComponent = Boolean(component) && !component.hasBeenSaved;
      removeComponentAction.hidden = !canRemoveNewComponent;
      removeComponentAction.disabled = !canRemoveNewComponent;
      if (!component) {
      ...
      if (component.type === 'search') {
        config.innerHTML = `<div class="style-config-form"><label>搜索-模板底纹词<input class="control" id="style-search-placeholder" value="${this.escape(component.placeholder || '')}" placeholder="请输入搜索-模板底纹词" /></label></div>`;
        ...
        const identityOptions = window.ConfigurationSections.identities;
        ...
        document.getElementById('resource-test-enabled').addEventListener('change', (event) => {
          updateTargeting({ testPlan: { ...targeting.testPlan, enabled: event.target.checked } });
          document.getElementById('resource-test-status').textContent = event.target.checked ? '生效' : '未生效';
        });
        ...
        config.innerHTML = `<div class="style-config-form"><label>信息流-记录名称<div class="control-with-tooltip"><input class="control" id="style-flow-title" value="${this.escape(component.title || '')}" placeholder="请输入信息流-记录名称" /><button class="help-tooltip" type="button" data-tooltip="仅后台可见" aria-label="信息流-记录名称说明">?</button></div></label><label>选择应用库<select class="control" id="style-flow-library"><option value="">请选择应用库</option><option value="活动商品库" ${component.library === '活动商品库' ? 'selected' : ''}>活动商品库（占位）</option><option value="精选商品库" ${component.library === '精选商品库' ? 'selected' : ''}>精选商品库（占位）</option><option value="高佣商品库" ${component.library === '高佣商品库' ? 'selected' : ''}>高佣商品库（占位）</option></select></label><div class="style-divider"><span>或直接填写</span></div><label>应用库 ID<input class="control" id="style-flow-library-id" value="${this.escape(component.libraryId || '')}" placeholder="请输入应用库 ID" /></label><label>应用库名称<input class="control" id="style-flow-library-name" value="${this.escape(component.libraryName || '')}" placeholder="请输入应用库名称" /></label></div>`;
    ...
    removeComponentAction.addEventListener('click', () => {
      const component = components.find((item) => item.id === this.activeComponentId);
      if (!component || component.hasBeenSaved) return;
      if (removeNewComponent(component.id)) window.BackofficeLayout.showToast('组件已移除');
    });
    ...
    document.getElementById('save-detail-template-style-page').addEventListener('click', () => {
      const invalidResource = components.find((component) => component.type === 'resource' && window.ConfigurationSections.validateTestPlan(component.targeting?.testPlan));
      if (invalidResource) {
        window.BackofficeLayout.showToast('测试计划校验失败', window.ConfigurationSections.validateTestPlan(invalidResource.targeting.testPlan));
        return;
      }
