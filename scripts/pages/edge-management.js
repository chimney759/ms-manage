window.EdgeManagementPage = {
  ...
  renderEditor(record, { copy = false } = {}) {
    ...
    root.innerHTML = `<header class="edge-form-title"><div><h2>${copy ? '复制贴边' : record.id ? '编辑贴边' : '添加贴边'}</h2><span>完善贴边展示的基础信息、素材与投放规则</span></div></header><form class="edge-form" data-edge-form novalidate><section class="shared-config-section"><h3>基础信息</h3>${field('业务', '<input class="control" value="美柚省钱App" disabled />')}${field('<b class="field-required">*</b>名称', `<input class="control" name="name" value="${this.escape(record.name)}" maxlength="40" placeholder="请输入贴边名称" required />`)}${field('<b class="field-required">*</b>排序值 <button class="help-tooltip" type="button" aria-label="排序值说明" data-tooltip="排序值越大，贴边展示越靠前。">?</button>', `<input class="control" name="sortValue" type="number" min="0" step="1" value="${this.escape(record.sortValue)}" placeholder="请输入排序值" required />`)}</section><section class="shared-config-section"><h3>素材信息</h3>${field('<b class="field-required">*</b>贴边图片', `<div class="edge-asset"><span class="edge-asset-preview">${record.image ? `<img src="${this.escape(record.image)}" alt="贴边图片" />` : '<b>图片</b>'}</span><span class="edge-asset-actions"><label class="button secondary edge-upload">上传图片<input type="file" accept="image/*" data-edge-image-input /></label><button class="edge-delete-image" type="button" data-edge-delete-image${record.image ? '' : ' disabled'}>删除图片</button></span><input type="hidden" name="image" value="${this.escape(record.image)}" /></div>`, 'edge-image-field')}</section>${routing}${frequencySection}${window.ConfigurationSections.renderTargeting({ prefix: 'edge', value: targeting, required: true, statusOptions: ['上线中', '待上线', '已下线'] })}${window.ConfigurationSections.renderTestPlan({ prefix: 'edge', value: testPlan, description: '测试 UID 在有效期内可看到该贴边配置，到期自动终止，不影响正式配置。' })}<footer class="edge-form-actions"><button class="button secondary" type="button" data-edge-cancel>取消</button><button class="button primary" type="submit">保存</button></footer></form>`;
  },
  bind({ activeTab = 'home', mode = 'list', recordId = '', copy = false } = {}) {
    ...
    if (mode === 'list') {
      ...
      root.querySelector('[data-edge-form]')?.addEventListener('submit', (event) => {
        ...
        targeting.status = root.querySelector('[name="edge-status"]:checked')?.value || '待上线';
        if (!targeting.onlineStart || !targeting.onlineEnd) { window.BackofficeLayout.showRequiredFieldToast('上下线时间'); return; }
