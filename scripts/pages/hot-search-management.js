window.HotSearchManagementPage = {
  ...
  createDraft(value = {}) {
    const defaults = {
      ...
      testPlan: { uids: '', start: '', end: '', enabled: false }, operator: '管理员', creator: '管理员', createdAt: '2026-08-16 10:00:00', editor: '管理员', updatedAt: '2026-08-16 10:00:00',
    };
  ...
  normalize(record) { const normalized = this.createDraft(record); return { ...normalized, creator: record.creator || '管理员', createdAt: record.createdAt || record.updatedAt || '2026-08-16 10:00:00', editor: record.editor || record.operator || '管理员', updatedAt: record.updatedAt || record.createdAt || '2026-08-16 10:00:00' }; },
  ...
  bind() {
    ...
    document.querySelector('[data-hot-search-sort]').addEventListener('click', (event) => { page.sort.direction = -page.sort.direction; const direction = page.sort.direction === 1 ? 'asc' : 'desc'; event.currentTarget.setAttribute('aria-sort', direction); event.currentTarget.innerHTML = `更新时间${page.sortIcon(direction)}`; renderTable(); });
    ...
    form.addEventListener('submit', (event) => {
      ...
      const existing = page.records.find((record) => record.id === page.editingId); const now = new Date().toLocaleString('zh-CN', { hour12: false }).replace(/\//g, '-'); const record = { ...page.draft, id: page.editingId || String(Math.max(0, ...page.records.map((item) => Number(item.id) || 0)) + 1), operator: existing?.operator || '管理员', creator: existing?.creator || '管理员', createdAt: existing?.createdAt || now, editor: '管理员', updatedAt: now };
  ...
  renderTable(filters = {}) {
    const name = (filters.name || '').trim().toLowerCase(); const visible = this.records.filter((record) => { const status = this.status(record); const dateStart = record.startAt.slice(0, 10); const dateEnd = record.endAt.slice(0, 10); return (!filters.app || record.app === filters.app) && (!name || record.name.toLowerCase().includes(name)) && (!filters.status || status === filters.status) && (!filters.start || dateEnd >= filters.start) && (!filters.end || dateStart <= filters.end); }).sort((left, right) => (Date.parse(String(left.updatedAt).replace(/-/g, '/')) - Date.parse(String(right.updatedAt).replace(/-/g, '/'))) * this.sort.direction);
    document.getElementById('hot-search-table-body').innerHTML = visible.map((record) => `<tr><td>${this.escape(record.id)}</td><td>${this.escape(record.name)}</td><td>${this.escape(record.startAt)}<br />- ${this.escape(record.endAt)}</td><td>${window.BackofficeLayout.statusTag(this.status(record))}</td><td>${window.BackofficeLayout.statusTag(record.enabled ? '启用' : '禁用')}</td><td class="hot-search-platform-cell">${this.platformText(record.platforms)}</td><td>${this.escape(record.sort)}</td><td>${this.escape(record.creator)}</td><td>${this.escape(record.createdAt)}</td><td>${this.escape(record.editor)}</td><td>${this.escape(record.updatedAt)}</td><td><button class="table-action" type="button" data-hot-search-edit="${this.escape(record.id)}">编辑</button></td></tr>`).join('');
