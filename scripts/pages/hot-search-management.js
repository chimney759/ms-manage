window.HotSearchManagementPage = {
  ...
  render() {
    const appOptions = this.apps.map((app) => `<option value="${this.escape(app)}">${this.escape(app)}</option>`).join('');
  ...
  bind() {
    ...
    const getFilters = () => ({ app: document.getElementById('hot-search-app').value, name: document.getElementById('hot-search-name').value, status: document.getElementById('hot-search-status').value, start: document.getElementById('hot-search-start').value, end: document.getElementById('hot-search-end').value });
    ...
    body.addEventListener('change', (event) => {
      if (event.target.matches('[data-word-field="jumpType"], [data-platform], #hot-search-test-enabled')) { page.syncDraftFromForm(form); redraw(); }
    });
    ...
    form.addEventListener('submit', (event) => {
      ...
      const testFilled = page.draft.testPlan.uids || page.draft.testPlan.start || page.draft.testPlan.end;
      const testComplete = !testFilled || Boolean(page.draft.testPlan.uids && page.draft.testPlan.start && page.draft.testPlan.end);
      const validations = [[page.draft.app, 'hot-search-form-app-row', 'APP'], [page.draft.name, 'hot-search-form-name-row', '活动名称'], [/^\d+$/.test(page.draft.sort) && Number(page.draft.sort) >= 1 && Number(page.draft.sort) <= 9999, 'hot-search-form-sort-row', '排序'], [validWords, 'hot-search-words-section', '热搜词'], [hasPlatforms, 'hot-search-form-platform-row', '平台和版本'], [page.draft.startAt && page.draft.endAt, 'hot-search-form-date-row', '上下线时间'], [testComplete, 'hot-search-test-date-row', '测试计划']];
  ...
  renderTable(filters = {}) {
    ...
    document.getElementById('hot-search-table-body').innerHTML = visible.map((record) => `<tr><td>${this.escape(record.id)}</td><td>${this.escape(record.name)}</td><td>${this.escape(record.startAt)}<br />- ${this.escape(record.endAt)}</td><td class="${this.status(record) === '上线中' ? 'status-online' : ''}">${this.status(record)}</td><td>${record.enabled ? '启用' : '禁用'}</td><td class="hot-search-platform-cell">${this.platformText(record.platforms)}</td><td>${this.escape(record.sort)}</td><td>${this.escape(record.operator)}</td><td><button class="table-action" type="button" data-hot-search-edit="${this.escape(record.id)}">编辑</button></td></tr>`).join('');
